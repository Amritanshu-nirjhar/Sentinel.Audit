/**
 * Sentinel.Audit - Multilingual Brand Impersonation & Unicode Anti-Evasion Engine
...
(function (global) {
  ...
  // 2. Unicode Normalization & Invisible Evasion Cleaner
  // Strips zero-width spaces, joiners, soft hyphens, and byte order marks
  ...
  function normalizeUnicodeText(input) {
    ...
    // Step 2: Strip invisible control and zero-width characters used to evade simple pattern matching
    normalized = normalized.replace(ZERO_WIDTH_REGEX, "");
    // Step 3: Collapse repeated spaces/whitespace
  ...
  const SCRIPT_RANGES = [
    {
      name: "Devanagari",
      regex: /[\u0900-\u097F]/,
      associatedLanguages: ["Hindi", "Marathi"],
      codeRange: "U+0900–U+097F"
    },
    {
      name: "Tamil",
      regex: /[\u0B80-\u0BFF]/,
      associatedLanguages: ["Tamil"],
      codeRange: "U+0B80–U+0BFF"
    },
    {
      name: "Telugu",
      regex: /[\u0C00-\u0C7F]/,
      associatedLanguages: ["Telugu"],
      codeRange: "U+0C00–U+0C7F"
    },
    {
      name: "Bengali",
      regex: /[\u0980-\u09FF]/,
      associatedLanguages: ["Bengali"],
      codeRange: "U+0980–U+09FF"
    },
    {
      name: "Latin",
      regex: /[a-zA-Z]/,
      associatedLanguages: ["English"],
      codeRange: "U+0041–U+007A"
    }
  ];
  ...
  function detectScripts(text) {
    ...
    for (let script of SCRIPT_RANGES) {
      if (script.regex.test(normalized)) {
        detected.push({
          name: script.name,
          languages: script.associatedLanguages,
          range: script.codeRange
        });
      }
    }
  ...
  function getScriptNames(text) {
    return detectScripts(text).map(s => s.name);
  }

  // 4. Domain Authenticity Checker
  function isLegitimateDomain(domain, brandObj) {
    if (!domain || !brandObj) return false;
    const cleanDomain = domain.toLowerCase().trim().replace(/^www\./, "");
    return brandObj.legitimateDomains.some(legit => {
      const cleanLegit = legit.toLowerCase().trim();
      return cleanDomain === cleanLegit || cleanDomain.endsWith("." + cleanLegit);
    });
  }

  // 5. Anti-Evasion Fuzzy & Substring Matcher
  // Cleans punctuation/spaces for partial matching (e.g. attacker inserting dots or hyphens between Indic characters)
  function buildFuzzyKey(str) {
    return normalizeUnicodeText(str)
      .toLowerCase()
      .replace(/[\s\-_.:,;/\\|~`!@#$%^&*()[\]{}<>?+=]/g, "");
  }
  ...
  function matchesKeyword(text, keyword) {
    ...
    const normText = normalizeUnicodeText(text).toLowerCase();
    const normKw = normalizeUnicodeText(keyword).toLowerCase();

    // Direct substring match after NFC normalization and zero-width strip
    if (normText.includes(normKw)) {
      return true;
    }

    // Fuzzy match: stripped of punctuation and spacing
    const fuzzyText = buildFuzzyKey(normText);
    const fuzzyKw = buildFuzzyKey(normKw);
    if (fuzzyKw.length >= 3 && fuzzyText.includes(fuzzyKw)) {
      return true;
    }
  ...
  /**
   * Main Inspection API: Match text against Multilingual Brand Catalog
   * 
   * @param {Object} context
   * @param {string} [context.title] - Scanned page title or payor/payee name
   * @param {string} [context.description] - Meta description or transaction note
   * @param {string} [context.domain] - Hostname of target destination
   * @param {string} [context.url] - Full canonical URL
   * @param {string} [context.favicon] - Favicon URL / hash
   * @returns {Object|null} Match result or null if no brand found
   */
  function inspectMultilingualBrand(context) {
    if (!context) return null;

    const rawTitle = context.title || "";
    const rawDesc = context.description || "";
    const domain = (context.domain || "").toLowerCase().trim();
    const url = context.url || "";

    // Normalize inputs using Unicode NFC
    const normTitle = normalizeUnicodeText(rawTitle);
    const normDesc = normalizeUnicodeText(rawDesc);
    const normUrl = normalizeUnicodeText(url);

    // Detect scripts in title and combined content
    const titleScripts = detectScripts(normTitle);
    const allScripts = detectScripts(`${normTitle} ${normDesc} ${normUrl}`);
    const scriptNames = Array.from(new Set(allScripts.map(s => s.name)));

    const matchCandidates = [
      { text: normTitle, location: "title", priority: 1 },
      { text: normDesc, location: "description", priority: 2 },
      { text: normUrl, location: "url", priority: 3 }
    ];

    for (let brandObj of BRAND_KEYWORD_MAP) {
      const isOfficial = isLegitimateDomain(domain, brandObj);

      // Inspect keywords across all supported languages
      const languages = ["hi", "ta", "te", "bn", "mr", "en"];
      for (let lang of languages) {
        const kws = brandObj.keywords[lang] || [];
        for (let kw of kws) {
          for (let candidate of matchCandidates) {
            if (candidate.text && matchesKeyword(candidate.text, kw)) {
              // Found a match! Determine primary script for this language
              let primaryScript = "Latin";
              if (lang === "hi" || lang === "mr") primaryScript = "Devanagari";
              else if (lang === "ta") primaryScript = "Tamil";
              else if (lang === "te") primaryScript = "Telugu";
              else if (lang === "bn") primaryScript = "Bengali";

              return {
                matched: true,
                brand: brandObj.brand,
                fullName: brandObj.fullName,
                ...
                isLegitimateDomain: isOfficial,
                domain: domain,
                detectedScripts: scriptNames,
                titleScripts: titleScripts.map(s => s.name),
                legitimateDomains: brandObj.legitimateDomains
              };
            }
          }
        }
  ...
  // Export module for browser and Node.js
  const BrandKeywordsModule = {
    BRAND_KEYWORD_MAP,
    LANGUAGE_NAMES,
    SCRIPT_RANGES,
  ...
  if (typeof module !== "undefined" && module.exports) {
    module.exports = BrandKeywordsModule;
  }

  if (typeof global !== "undefined") {
    global.BrandKeywords = BrandKeywordsModule;
    global.BRAND_KEYWORD_MAP = BRAND_KEYWORD_MAP;
  }
